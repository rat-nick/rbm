...
class RBM:
    ...
    def __init__(
        ...
        self.v = torch.randn(n_visible, device=self.device)
        self.h = torch.randn(n_hidden, device=self.device)
    ...
    def apply_gradient(
        ...
        # print(torch.sum(activations, 0).item() / (len(minibatch) * self.h.shape[0]))
        # plt.hist(y=activations, bins=len(activations))
        # plt.show()
        # plt.clf()
        # plt.hist(self.h)
        # plt.show()
